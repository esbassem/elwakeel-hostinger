...
const PartnersTab = ({ currentUser }) => {
  const [partners, setPartners] = useState([]);
  const [searchQuery, setSearchQuery] = useState('');
  const [filterType, setFilterType] = useState('all');
  const [showQuickAdd, setShowQuickAdd] = useState(false);
  const [showFullAdd, setShowFullAdd] = useState(false);
  const { fetchPartners, deletePartner, getMissingFields, loading } = usePartners();
  ...
  const handleDelete = async (e, id, name) => {
    ...
    if (!isAdmin) {
      toast({
        title: "غير مصرح",
        description: "فقط المدير يمكنه حذف الشركاء",
        variant: "destructive"
      });
      return;
    }

    const confirmed = window.confirm(`هل أنت متأكد من حذف شريك \"${name}\"?`);
    if (!confirmed) return;

    const { error } = await deletePartner(id, false);
    if (!error) {
      loadPartners();
    }
  };
    ...
    <div className="max-w-6xl mx-auto space-y-6 pb-24">
      ...
      >
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 mb-6">
          <div>
            <h2 className="text-2xl font-black text-stone-800 mb-1">إدارة الشركاء</h2>
            <p className="text-sm text-stone-500">إجمالي {partners.length} شريك</p>
          </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white rounded-3xl border border-stone-100 shadow-sm">
          ...
          <p className="text-stone-500 font-bold">لا يوجد شركاء</p>
          <p className="text-sm text-stone-400 mt-1">ابدأ بإضافة شريك جديد</p>
        </div>
      ) : (
              ...
              >
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1 min-w-0">
                    <h3 className="text-lg font-bold text-stone-800 truncate mb-1 group-hover:text-indigo-600 transition-colors">{partnerName}</h3>
                    {partner.nickname && (
                      <p className="text-xs text-stone-500 truncate">\"{partner.nickname}\"</p>
                    )}
                  </div>
                  <div className="flex gap-1">
                    {isAdmin && (
                      <button
                        onClick={(e) => handleDelete(e, partner.id, partnerName)}
                ...
                <div className="space-y-2 mb-4">
                  {partner.phone1 && (
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className="text-stone-600 font-medium dir-ltr">{partner.phone1}</span>
                    </div>
                  )}
                  {partner.address && (
                    <div className="flex items-start gap-2 text-sm">
                      <MapPin className="w-3.5 h-3.5 text-stone-400 shrink-0 mt-0.5" />
                      <span className="text-stone-600 font-medium line-clamp-2">{partner.address}</span>
                    </div>
                  )}
                </div>
