...
const PartnerSearchBar = () => {
  ...
  useEffect(() => {
    const searchTimer = setTimeout(async () => {
      if (query.trim().length >= 1) {
        setIsSearching(true);
        const { data } = await fetchPartners({ search: query });
        setResults(data || []);
  ...
  const handleResultClick = (partnerId) => {
        ...
        {showResults && (
          <div className="absolute top-full right-0 left-0 mt-2 bg-white rounded-2xl shadow-2xl border border-stone-100 overflow-hidden z-50 max-h-80 overflow-y-auto">
            {results.length > 0 ? (
              <div className="py-2">
                {results.map((partner) => {
                  const displayName = partner.nickname || partner.name || 'بدون اسم';
                  const displayChar = (displayName || '?').charAt(0);
                  const partnerType = partner.account_type || 'customer';
                  
                  return (
                    <div
                      key={partner.id}
                      onClick={() => handleResultClick(partner.id)}
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold border flex-shrink-0 ${
                          partnerType === 'customer' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' :
                          partnerType === 'supplier' ? 'bg-indigo-50 text-indigo-600 border-indigo-100' :
                        ...
                        <div className="flex flex-col">
                          ...
                          {partner.phone1 && (
                            <div className="flex items-center gap-1.5 text-xs text-stone-400 dir-ltr">
                              ...
                              <span className="font-mono">{partner.phone1}</span>
                            </div>
                          )}
                        </div>
                      </div>
                      <div className="text-xs font-bold text-stone-300 flex-shrink-0">
                        {partnerType === 'customer' ? 'عميل' : partnerType === 'supplier' ? 'مورد' : 'مشترك'}
                      </div>
                    </div>
                  );
                })}
